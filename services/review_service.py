...
async def _call_ollama(prompt: str, max_retries: int = 3) -> str:
    """
    Core Ollama caller. Sends a prompt to the local Ollama API and returns
    the raw text response. Retries on transient failures.
    """
    payload = {
        "model": OLLAMA_MODEL,
...
def _extract_json(text: str) -> str:
    ...
    or surrounding prose. Ollama models sometimes wrap JSON in ```json blocks.
    """
...
async def ask_ollama_to_review(files_data: list[dict], max_retries: int = 3) -> str:
    """
    Queries the local Ollama model with a structured analysis prompt to parse
    vulnerabilities, logic traps, and syntax gaps from a list of files.
    Replaces ask_gemini_to_review.
    """
    diff_context = ""
...
async def ask_ollama_triage(files_metadata: list[dict], max_retries: int = 3) -> str:
    """
    Sends files with their heuristic scores to Ollama and asks it to re-rank
    and flag anything the heuristics might have missed.
    Replaces ask_gemini_triage.
    """
    context_str = ""
